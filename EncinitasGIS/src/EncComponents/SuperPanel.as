...
{
	import EncComponents.SuperPanelEvent;
	...
	[Event(name="maximize", type="EncComponents.SuperPanelEvent")]
	/**
	...
	[Event(name="minimize", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="restore", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="dragStart", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="drag", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="dragEnd", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="resizeStart", type="EncComponents.SuperPanelEvent")]
	 ...
	 */
	[Event(name="resizeEnd", type="EncComponents.SuperPanelEvent")]
	
	
	[Event(name="test", type="EncComponents.SuperPanelEvent")]
	...
	public class SuperPanel extends Panel
	{
		...
		/**
		 *  The current released version of the component, encoded as a uint.
		 */
		...
		public function SuperPanel()
		{
			...
			this.addEventListener(SuperPanelEvent.TEST,testEventHandler);
		...
		/**
		 *  @private 
		 * Drew 3/3/2011 Added to map a map object part of the core superpanel
		 */
		...
		{
			// If we add this event listener when we're a pop-up then
			// controls like the ComboBox fail to function properly.
			if (!isPopUp)
			{
		...
		private function testEventHandler(event:Event):void
		{
		...
		/**
		 *  Default handler for MouseEvent.MOUSE_DOWN events fired from this
		 *  container.
		 * 
		...
		override public function validateDisplayList():void
		{
			...
			// Don't allow the resizeHitBox and either of the ScrollBars
			// overlap each other.
			...
			if (verticalScrollBar)
			{
				...
				verticalScrollBar.setActualSize(verticalScrollBar.width, 
					Math.min(h - resizeHitBox.height, 
						h - verticalScrollBar.width));
			}
			...
			if (horizontalScrollBar)
			{
				...
				horizontalScrollBar.setActualSize(Math.min(w - resizeHitBox.width, 
					w - horizontalScrollBar.height), 
