...
{
	import EncComponents.TOC.toc.tocClasses.*;
	import EncComponents.TOC.toc.utils.MapUtil;
	...
	public class TocMapLayerItem extends TocItem
	{
		...
		private var _isVisibleLayersSet:Boolean = true;
		private var _map:Map;
		
	    public function TocMapLayerItem(layer:Layer, targetMap:Map, labelFunction:Function = null, isMapServiceOnly:Boolean = false, excludedLayers:ArrayCollection = null, showMeta:Boolean = false, iscollapsed:Boolean = false)
	    {
	        super();
			_legendData = new ArrayCollection([]);
	        _layer = layer;
			_map =  targetMap //ViewerContainer.getInstance().mapManager.map;
			metaBtnVisible = showMeta;
	        ...
	        if (!isMapServiceOnly)
	        {
	            if (layer.loaded)
	            {
					if (layer is FeatureLayer){
						...
						//fl.addEventListener(FeatureLayerEvent.QUERY_FEATURES_COMPLETE, function(evt:FeatureLayerEvent):void{trace(evt.featureSet.features.length.toString());});
						minScale = fl.minScale;
						maxScale = fl.maxScale;
//						if(fl.definitionExpression != ""){
//							trace(fl.numGraphics.toString());
//							trace(fl.graphicProvider.length.toString());
//							layerExtent = GraphicUtil.getGraphicsExtent(fl.graphicProvider.toArray());
//						}else{
...
//						}
			...
			_map.addEventListener(ExtentEvent.EXTENT_CHANGE,checkExtent);
	    }
...
/*						if (actualVisibleLayers.indexOf(layerInfo.id) != -1)
				...
				createKMLLayerTocItems(this, KMLLayer(layer),_map, _excludedLayers, _iscollapsed);
	        ...
	        if (layerInfos){
	            ...
	            for each (var layerInfo1:LayerInfo in rootLayers){
					var tlii:TocLayerInfoItem = createTocLayer(this, layerInfo1, layerInfos, visibleLayers, layer, _excludedLayers, _iscollapsed, _map );
		...
		public function set maxScale( value:Number ):void
		{
			...
			if(_maxScale > 0 && _minScale > 0){
				if ((this.map.scale >= _maxScale) &&
					(this.map.scale <= _minScale)){
		...
		private static function createKMLLayerTocItems(parentItem:TocItem, layer:KMLLayer, map:Map, excludeLayers:ArrayCollection = null, iscollapsed:Boolean = false):void
		{
			if(layer.folders.length == 0 && layer.networkLinks.length == 0){
				...
				if (layerInfos){
					var rootLayers:Array = findRootLayers(layerInfos);
					for each (var layerInfo1:LayerInfo in rootLayers){
						var tlii:TocLayerInfoItem = createTocLayer(parentItem, layerInfo1, layerInfos, visibleLayers, layer, excludeLayers, iscollapsed, map);
			...
			for each (var folder:KMLFolder in layer.folders)
			{
				if (folder.parentFolderId == -1)
					parentItem.addChild(createKmlFolderTocItem(parentItem, folder, layer.folders, layer, map));
			}
			...
			for each (var networkLink:KMLLayer in layer.networkLinks)
			{
				...
				if (!(hasParentFolder(Number(networkLink.id), layer.folders)))
				{
					...
					if (networkLink.loaded)
						parentItem.addChild(createKmlNetworkLinkTocItem(parentItem, networkLink, layer, map));
					else
	    ...
	    private static function createTocLayer(parentItem:TocItem, layerInfo:LayerInfo, layerInfos:Array, visibleLayers:Array, tlayer:Layer, excludeLayers:ArrayCollection, iscollapsed:Boolean, map:Map):TocLayerInfoItem
	    {
	        const item:TocLayerInfoItem = new TocLayerInfoItem(parentItem, layerInfo, visibleLayers, map);
			item.scroller = parentItem.scroller;
			item.tocMinWidth = parentItem.tocMinWidth;
			item.metatooltip = parentItem.metatooltip;
			item.metaBtnVisible = parentItem.metaBtnVisible;
			...
			if (tlayer is ArcGISTiledMapServiceLayer) {
				ArcGISTiledMapServiceLayer(tlayer).getDetails(layerInfo.id, new AsyncResponder(
					function myResultFunction(result:LayerDetails, token:Object = null):void
					{
						...
						if(item.metaBtnVisible && (result.description || result.copyright))
				...
				if(item.metaBtnVisible && (fl.layerDetails.description || fl.layerDetails.copyright))
	        ...
	        if (layerInfo.subLayerIds){
	            for each (var childId:Number in layerInfo.subLayerIds){
	                ...
	                if (childLayer){
						var tlii:TocLayerInfoItem = createTocLayer(item, childLayer, layerInfos, visibleLayers, tlayer, excludeLayers, iscollapsed, map);
		...
		private static function createKmlFolderTocItem(parentItem:TocItem, folder:KMLFolder, folders:Array, layer:KMLLayer, map:Map):TocKmlFolderItem
		{
			...
			if (folder.subFolderIds && folder.subFolderIds.length > 0){
				...
				for each (var childId:Number in folder.subFolderIds){
					...
					if (childFolder)
						item.addChild(createKmlFolderTocItem(item, childFolder, folders, layer, map));
				}
				...
				for each (var featureInfo:KMLFeatureInfo in folder.featureInfos){
					if (featureInfo.type == KMLFeatureInfo.NETWORK_LINK){
						...
						item.addChild(createKmlNetworkLinkTocItem(item, networkLink, layer, map));
					}
				}
			}
			return item;
		...
		private static function createKmlNetworkLinkTocItem(item:TocItem, networkLink:KMLLayer, layer:KMLLayer, map:Map):TocKmlNetworkLinkItem
		{
			var tocKmlNetworkLinkItem:TocKmlNetworkLinkItem = new TocKmlNetworkLinkItem(item, networkLink, layer);
			if (networkLink.loaded)
				createKMLLayerTocItems(tocKmlNetworkLinkItem, networkLink, map); // as network link is also a type of KMLLayer
			else
				networkLink.addEventListener(LayerEvent.LOAD, layerLoadHandler);
			
			function layerLoadHandler(event:LayerEvent):void
			{
				createKMLLayerTocItems(tocKmlNetworkLinkItem, networkLink, map);
			}
