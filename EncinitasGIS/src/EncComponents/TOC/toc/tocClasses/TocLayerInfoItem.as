...
package EncComponents.TOC.toc.tocClasses
{
	...
	{
	    public function TocLayerInfoItem(parentItem:TocItem, layerInfo:LayerInfo, visibleLayers:Array, map:Map)
	    {
	        super(parentItem);
			...
			if (_Map)
			{
				...
				_Map.addEventListener(MapEvent.LAYER_ADD,layeradded);
			}
		...
		public function set maxScale( value:Number ):void
		{
			...
			if(_maxScale > 0 && _minScale > 0){
				if ((_Map.scale >= _maxScale) &&
					(_Map.scale <= _minScale)){
		...
		private function layeradded(evt:LayerEvent):void{
			trace (evt.layer.id);
		}
		private function checkExtent(evt:ExtentEvent):void{
			...
			{
				if (evt){
					if (evt.target is Map)
					{
						targetMap = evt.target as Map;
					}
				}
			...
			if(_maxScale > 0 && _minScale > 0){
				if ((targetMap.scale >= _maxScale) &&
					(targetMap.scale <= _minScale)){
					this.scaledependant = false;
				}			
			} else if (_maxScale > 0 ){
				if ((targetMap.scale >= _maxScale)){
		...
		//--------------------------------------------------------------------------
		//  Property:  layerInfo
		// Since we will not be using the viewer we cannot listen to viewer events. Instead we
		//set a public property Map so we can disten directly to map events and not have
		//to rely on viewer events
		//Drew 11/23/2011
		//--------------------------------------------------------------------------
